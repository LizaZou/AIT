...
if 350 <= size_cluster < 1100:  # size of 2 to 5 trees cluster in pixel
     R1 = 0.20  # first area  
     R2 = 0.14  # second area
     R3 = 0.09  # third area 
    ...
    for col in range(globals()['matF' + str(h)].shape[1]):
        for lig in range(globals()['matF' + str(h)].shape[0]):
            ...
            if (globals()['matF' + str(h)][lig, col] == 100):
                globals()['matF' + str(h)][lig, col] = 0.25  # /!\ MODIFY 
...
rel_path = os.path.join("..", "ToulouseFinal/ArbreAprèsTemp3.csv")
file_path = os.path.join(root, rel_path)
